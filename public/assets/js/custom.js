...
$(document).ready(function () {
  $(this).on("click", "#front-spaces", function () {
    let btn = $(this);
    $("error-name").empty();
    const formData = new FormData(document.querySelector("#front-add-space"));
    $.ajax({
      url: route("add-space"),
      type: "post",
      data: formData,
      ...
      success: (res) => {
        if (res.success == true) {
          ...
          let html = "";
          if (res.errors.space_title != undefined) {
            html += res.errors.space_title[0];
          }
          if (res.errors.description != undefined) {
            html += "<br />" + res.errors.description[0];
          }
          $(".error-name").html(html);
          btn.text("Create");
          btn.prop("disabled", false);
        }
      },
  ...
  $(this).on("click", ".reaction", function () {
    ...
    $.ajax({
      ...
      success: (res) => {
        if (res.success) {
          reactionBtn.html(`<i class="${reaction}"></i> React ${res.count}`);
          bootstrap.Collapse.getInstance($(`#collapseExample-${id}`)).hide();
        }
      },
    });
  });
  ...
  $(this).on("keyup", "#search", function () {
    if ($("#search").val().length > 2)
      $.ajax({
        ...
        beforeSend: () =>
          $("#search-loading").html("<i class='fa fa-spin fa-spinner'></i"),
        success: (res) => {
  ...
  $(this).on("change", "#post-edit-file-input", (e) => {
    ...
    reader.onload = () =>
      $("#img-preview-edit").attr("src", reader.result).removeClass("d-none");
